...
export const mintNFT = ({ Tezos, amount, metadata, prompt }) => {
  return async (dispatch) => {
    try {
      const contract = await Tezos.wallet.at(config.contractAddress);
      let bytes = "";
      ...
      const op = await contract.methods.mint(bytes, prompt).send();
      await op.confirmation();
      dispatch(fetchData());
    } catch (e) {
      console.log(e);
...
export const collectNFT = ({ Tezos, amount, id }) => {
  return async (dispatch) => {
    try {
      ...
      const op = await contract.methods.collect(id).send({mutez : true, amount: amount});
      await op.confirmation();
      dispatch(fetchData());
    } catch (e) {
      console.log(e);
